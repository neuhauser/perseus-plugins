...
namespace ProteomicRuler
{
    public class CopyNumbers : IMatrixProcessing
    {
        public bool HasButton { get { return false; } }
        public Image ButtonImage { get { return null; } }
        public string HelpDescription { get { return "Estimate cellular copy numbers from protein intensities using the proteomic ruler approach.\n\n"
            + "Requirements:\nA deep proteomic dataset of either " + String.Join(", ",SupportedOrganismNames()) + ", generated from a nucleated cell type or tissue, processed with a recent uniprot database.\n\n"
            + "Parameters:\n\n"
            + "Intensities: A series of columns containing the protein intensities in the different samples.\nIf you want to use LFQ intensities, it is recommended to process the dataset with 1 min. LFQ ratio count.\n\n"
            + "Averaging mode: Select how to handle multiple columns. By default, all columns will be treated independently. Alternatively, enforce the same normalization for all columns or for groups of replicates. "
            + "This will ensure that equal intensity values get translated into equal copy numbers. This mode is mainly useful for LFQ intensities, which are already normalized to ensure comparability across samples."
            + "However, the samples will then not strictly have histone amounts or total protein amounts corresponding to the expected or user-defined values. "
            + "As a last option, all selected columns can be averaged prior to estimating copy numbers. This is useful e.g. for a set of technical replicates."
            + "Detectability normalization: By default, the algorithm assumes direct proportionality of the MS intensity and the protein mass. In other words, the molecular mass serves as normalization factor to calculate copy numbers from intensities. "
            + "Optionally, one can introduce another level of normalization using e.g. the number of theoretical peptides as used in the iBAQ method. Just select a column containing normalization factors of your choice.\n\n"
            + "Scaling mode: Select whether you want to scale the total protein amount using the histone proteomic ruler or if you want to define a total protein mass per cell.\n\n"
            + "Total cellular protein concentration: To translate protein mass per cell into cell volume, an estimate of the total cellular protein concentration is needed. This value varies only to a small extent between different cell types as diverse as E. coli, yeast or human cells."
            ; } }
        ...
        const double _avogadro = 6.02214129e23;
        const double _basePairWeight = 615.8771;
            ...
            ref IDocumentData[] documents, ProcessInfo processInfo)
        {
            ...
            {
                double[] values = new double[mdata.RowCount];
            ...
            {
                double[] logBases = new double[] { 2, Math.E, 10 };
                double logBase =
                ...
                for (int col = 0; col < columns.Count; col++)
                    for (int row = 0; row < mdata.RowCount; row++)
                    {
                        ...
                        columns[col][row] = Math.Pow(logBase, columns[col][row]);
                    }
            ...
            if (param.GetBoolWithSubParams("Detectability correction").Value)
                detectabilityNormFactor = mdata.NumericColumns[param.GetBoolWithSubParams("Detectability correction").GetSubParameters().GetSingleChoiceParam("Correction factor").Value];
            ...
            double cValue = (organism.GenomeSize * _basePairWeight) / _avogadro;
            ...
            for (int row = 0; row < mdata.RowCount; row++)
                histoneCol[row] = (ArrayUtils.Contains(histoneRows, row)) ? new string[] { "+" } : new string[] { "" };
            ...
            for (int col = 0; col < columns.Count; col++)
            {
                ...
                switch (param.GetSingleChoiceWithSubParams("Scaling mode").Value)
                {
                    case 0: // total protein amount
                        ...
                        factor = (param.GetSingleChoiceWithSubParams("Scaling mode").GetSubParameters().GetDoubleParam(
                                "Protein amount per cell [pg]").Value * 1e-12 * _avogadro) / mwWeightedNormalizedSummedIntensities;
                        break;
                    case 1: // histone mode
                        ...
                        double ploidy = param.GetSingleChoiceWithSubParams("Scaling mode").GetSubParameters().GetDoubleParam("Ploidy").Value;
                        factor = (cValue * ploidy * _avogadro) / mwWeightedNormalizedSummedHistoneIntensities;
                        break;
                    default:
                        factor = 1;
                        break;
            ...
            {
                if (param.GetSingleChoiceWithSubParams("Averaging mode").GetSubParameters().GetSingleChoiceParam("Grouping").Value == -1)
                {
                    processInfo.ErrString = "No grouping selected.";
                    return;
                }
                string[][] groupNames = mdata.GetCategoryRowAt(param.GetSingleChoiceWithSubParams("Averaging mode").GetSubParameters().GetSingleChoiceParam("Grouping").Value);
                string[] uniqueGroupNames = Unique(groupNames);
                int[] grouping = new int[columns.Count];
                for (int i = 0; i < columns.Count; i++)
                {
                    if (intensityCols[i] >= mdata.ExpressionColumnCount)
            ...
            for (int col = 0; col < columns.Count; col++)
            {
                ...
                for (int row = 0; row < mdata.RowCount; row++)
                    if (!double.IsNaN(column[row]) && !double.IsNaN(mw[row]))
                    {
                        ...
                        totalProtein += (copyNumbers[row] * mw[row] * 1e12) / _avogadro; // picograms
                        if (ArrayUtils.Contains(histoneRows, row))
                            histoneMass += (copyNumbers[row] * mw[row] * 1e12) / _avogadro; // picograms
                    }

                double totalVolume = (totalProtein / (param.GetDoubleParam("Total cellular protein concentration [g/l]").Value)) * 1000; // femtoliters

                for (int row = 0; row < mdata.RowCount; row++)
                    if (!double.IsNaN(column[row]) && !double.IsNaN(mw[row]))
                    {
                        concentrations[row] = ((copyNumbers[row] / (totalVolume * 1e-15)) / _avogadro) * 1e9; // nanomolar
                        massFraction[row] = (((copyNumbers[row] * mw[row] * 1e12) / _avogadro) / totalProtein) * 1e6; // ppm
                ...
                if (intensityCols[col] < mdata.ExpressionColumnCount && param.GetSingleChoiceWithSubParams("Averaging mode").Value != 3)
                {
        ...
        private string[] Unique(string[][] x)
        {
        ...
        private IList<double> ExtractValidValues(double[] values, bool zerosAreValid)
        {
            ...
            return new Parameters(
                new Parameter[] {
                    ...
                    new MultiChoiceParam("Intensities") {
                        Help = "Specify the columns that contain the intensities to be used for copy number estimation. If several columns are selected, the method will calculate the median.",
                        Values = ArrayUtils.Concat(mdata.ExpressionColumnNames, mdata.NumericColumnNames),
                        Value = Match(ArrayUtils.Concat(mdata.ExpressionColumnNames,mdata.NumericColumnNames),new []{"intensit"},false, true, false)
                    },
                    new SingleChoiceWithSubParams("Averaging mode",0) {
                        Values = new string[]{"All columns separately","Same normalization for all columns","Same normalization within groups","Average all columns"},
                        Help = "Select how multiple columns will be treated",
                        SubParams = new List<Parameters>() {
                            new Parameters( new Parameter[] {}),
                            new Parameters( new Parameter[] {}),
                            new Parameters( new Parameter[] { new SingleChoiceParam("Grouping") { Values = mdata.CategoryRowNames, Value = Match(mdata.CategoryRowNames.ToArray(), new string[] {"group"}, false, true, true )[0] } }),
                            new Parameters( new Parameter[] {})
                        }
                    },
                    ...
                    new BoolWithSubParams("Detectability correction",false) {
                        Help = "Without correction, the algorithm assumes linearity between the signal and the mass of the proteins.\nOptionally select protein-specific correction factors such as the number of theoretical peptides.",
                        SubParamsFalse = new Parameters( new Parameter[] {}),
                        SubParamsTrue = new Parameters( new Parameter[] {
                    ...
                    new MultiChoiceParam("Output") {
                        ...
                        Values = new [] {"Copy number per cell", "Concentration [nM]", "Relative abundance (mass/total mass)", "Relative abundance (molecules/total molecules)", "Copy number rank", "Relative copy number rank", "Sample summary (total protein, total molecules, cell volume, ...)"},
                        Value = new [] {0, 6}
                    }
                }
                );
        ...
        private int[] Match(string[] haystack, string[] needles, bool caseSensitive, bool matchSubstring, bool matchFirstIfNothingFound)
        {
        ...
        private static Organism[] SupportedOrganisms()
        {
            ...
            hSapiens.Name = "H. sapiens";
            hSapiens.GenomeSize = 3200000000;
            hSapiens.HistoneIds = new string[]
                                      {
                                          "P07305", "Q8IZA3", "Q92522", "P0C5Y9", "P0C5Z0", "H0YFX9", "Q9BTM1", "A8MQC5",
                                          "C9J0D1", "C9J386", "E5RJU1", "Q71UI9", "P16104", "B4DJC3", "D6RCF2", "O75367",
                                          "Q5SQT3", "Q9P0M6", "P0C0S5", "P0C1H6", "A9UJN3", "P57053", "Q7Z2G1", "B4DEB1",
                                          "P84243", "B2R4P9", "K7EMV3", "K7ES00", "K7EK07", "K7EP01", "Q6NXT2", "Q02539",
                                          "P16401", "P16403", "P16402", "Q4VB24", "P10412", "A3R0T8", "A1L407", "P22492",
                                          "Q96QV6", "P04908", "Q08AJ9", "Q93077", "P20671", "P0C0S8", "A3KPC7", "Q96KK5",
                                          "Q99878", "A4FTV9", "Q92646", "Q96A08", "P33778", "P62807", "P58876", "B2R4S9",
                                          "Q93079", "P06899", "O60814", "Q99880", "I6L9F7", "Q99879", "Q99877", "P23527",
                                          "P68431", "P62805", "Q99525", "Q0VAS5", "B2R4R0", "Q6FI13", "Q8IUE6", "Q16777",
                                          "Q16778", "B4DR52", "Q5QNW6", "Q71DI3", "Q5TEC6", "Q7L7L0", "Q8N257", "Q16695",
                                          "Q6TXQ4", "Q14463", "B4E0B3", "B2R5B6", "A2RUA4", "B2R5B3", "Q9HA11", "A8K9J7",
                                          "B2R6Y1", "B4E380", "A8K4Y7", "Q6B823", "Q6LBZ2", "A3R0T7"
                                      };
            organisms.Add(hSapiens);

            Organism mMusculus = new Organism();
            mMusculus.Name = "M. musculus";
            mMusculus.GenomeSize = 2700000000;
            mMusculus.HistoneIds = new string[]
                                       {
                                           "Q9DAD9", "B2RTM0", "Q8CBB6", "Q921L4", "Q5M8Q2", "Q810S6", "B1AV31", "Q497L1",
                                           "A9Z055", "Q8CGP9", "P10922", "Q8CJI4", "E0CZ52", "E0CYL2", "Q8VIK3", "Q80ZM5",
                                           "Q9CQ70", "Q8R1M2", "Q3THW5", "Q8R029", "B2RVP5", "P27661", "Q9QZQ8", "Q8CA90",
                                           "Q8BP16", "Q9CTR1", "Q8CCK0", "Q9D3V6", "Q9D3U7", "Q3UA95", "Q3TFU6", "G3UWL7",
                                           "G3UX40", "P0C0S6", "F8WI35", "E0CZ27", "E0CYN1", "E0CYR7", "P84244", "P02301",
                                           "Q9QYL0", "P43275", "P43276", "P15864", "Q5SZA3", "P43277", "Q149Z9", "P43274",
                                           "Q07133", "I7HFT9", "Q8CGP4", "P22752", "B2RVF0", "Q61668", "Q8CGP5", "A0AUV1",
                                           "Q8CGP6", "A3KPD0", "Q8CGP7", "F8WIX8", "A0JNS9", "P70696", "Q64475", "Q6ZWY9",
                                           "P10853", "Q64478", "A0JLV3", "Q8CGP1", "B2RVD5", "P10854", "B2RTK3", "Q8CGP2",
                                           "P68433", "P84228", "A1L0U3", "A1L0V4", "P62806", "B2RWH3", "Q6GSS7", "Q64522",
                                           "Q64523", "Q149V4", "Q64525", "G3X9D5", "Q64524", "B9EI85", "Q61667", "Q8BFU2",
                                           "A2AB79", "Q9D2U9", "Q8CGP0", "Q6B822", "P07978", "Q9D9Z7"
                                       };
            organisms.Add(mMusculus);

            Organism dMelanogaster = new Organism();
            dMelanogaster.Name = "D. melanogaster";
            dMelanogaster.GenomeSize = 130000000;
            dMelanogaster.HistoneIds = new string[]
                                           {
                                               "Q6TXQ1", "P02255", "Q4AB54", "Q4ABE3", "Q4ABD8", "Q4AB94", "P84051",
                                               "Q4AB57", "P08985", "P02283", "P02299", "E2QCP0", "P84249", "P84040"
                                           };
            organisms.Add(dMelanogaster);

            Organism cElegans = new Organism();
            cElegans.Name = "C. elegans";
            cElegans.GenomeSize = 100300000;
            cElegans.HistoneIds = new string[]
                                      {
                                          "P10771", "P15796", "Q19743", "O17536", "O01833", "Q9U3W3", "Q18336", "P09588",
                                          "J7S164", "J7SA65", "Q27485", "Q23429", "Q27511", "P04255", "Q27894", "P08898",
                                          "K7ZUH9", "Q10453", "Q9U281", "Q27490", "Q27532", "P62784", "Q27484", "Q27876",
                                          "O16277", "Q27489"
                                      };
            organisms.Add(cElegans);

            Organism sCerevisiae = new Organism();
            sCerevisiae.Name = "S. cerevisiae";
            sCerevisiae.GenomeSize = 12100000;
            sCerevisiae.HistoneIds = new string[]
                                         {
                                             "P53551", "P04911", "P04912", "Q12692", "P02293", "P02294", "P61830", "P02309"
                                         };
            organisms.Add(sCerevisiae);

            Organism sPombe = new Organism();
            sPombe.Name = "S. pombe";
            sPombe.GenomeSize = 14100000;
            sPombe.HistoneIds = new string[] { "P48003", "P04909", "P04910", "P04913", "P09988", "P10651", "P09322" };
